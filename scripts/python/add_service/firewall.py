...
def open_service_port(port, protected_ports):

    """ Öffnet einen Port über ufw (Uncomplicated Firewall). """

    if is_port_protected(port, protected_ports):
        print_error(logger, f"Port {port}/tcp ist geschützt und darf nicht geöffnet werden.")
        return False
    else: 
        try:
